---
id: index
title: Developer Patterns with Vault and Kubernetes
sidebar_label: Introduction
---

In the ideal world application secrets have the following criteria:
...
In this exampl, we will look at how the Vault integration for Kubernetes allows an operator or developer to use metadata annotations to inject dynamically generated static secrets, database secrets, x509 certificates, and how to use Vaults encryption as a service. The integration automatically handles all the authentication with Vault and the management of the secrets, the application just reads the secrets from the filesystem.
...
When dealing with a vanilla Kubernetes approach often you have solution which looks something like the following.
```yaml
apiVersion: v1
kind: Secret
metadata:
...
If you only need static secrets and are happy with using Environment Variables for your secrets then you may be ok. However, applications often require more than just static secrets. You find yourself requiring the following features.

* Unique usernames and passwords for database access
* Static secrets such as API Keys
* X509 certificates for securing API endpoints with TLS
* Ability to encyrpt data

This is where HashiCorp Vault comes into the picture, for example, we can use Vault to dynamically generate database credentials for a PostgreSQL database. Adding the annotations shown in the following example automatically inject secrets controlled by the db-creds role into the pod.
